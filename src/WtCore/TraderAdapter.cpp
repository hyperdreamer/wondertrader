...
TraderAdapter::TraderAdapter(EventNotifier* caster /* = NULL */)
    : _id(""), _cfg(NULL), _state(AS_NOTLOGIN), _trader_api(NULL), _orders(NULL), _stat_map(NULL), _risk_mon_enabled(false), _save_data(false), _notifier(caster), _ignore_sefmatch(false)
{
}
...
uint32_t TraderAdapter::doEntrust(WTSEntrust* entrust)
{
    ...
    if (cInfo == NULL)
        cInfo = getContract(entrust->getCode());
