...
HisDataReplayer::HisDataReplayer()
    : _listener(NULL), _cur_date(0), _cur_time(0), _cur_secs(0), _cur_tdate(0), _tick_enabled(true), _opened_tdate(0), _closed_tdate(0), _tick_simulated(true), _running(false), _begin_time(0), _end_time(0), _bt_loader(NULL), _min_period("d"), _cache_clear_days(0), _align_by_section(false)
{
}
...
bool HisDataReplayer::loadStkAdjFactorsFromLoader()
{
    ...
    if (ret)
        WTSLogger::info("Adjusting factors of {} contracts loaded via extended loader", _adj_factors.size());
    return ret;
}
...
uint32_t HisDataReplayer::locate_barindex(const std::string& key, uint64_t now, bool bUpperBound /* = false */)
{
    ...
    if (it == barsList->_bars.end())
        ...
        if (bUpperBound) { // 如果是找上边界，则要比较时间向下修正，因为lower_bound函数找的是大于等于curTime的K线
...
double HisDataReplayer::calc_fee(const char* stdCode, double price, double qty, uint32_t offset)
{
    ...
    if (fItem._by_volume) {
        ...
        case 0:
            ...
            break;
        case 1:
            ret = fItem._close * qty;
            break;
        case 2:
            ret = fItem._close_today * qty;
            break;
        default:
            ret = 0.0;
            break;
        ...
        switch (offset) {
        case 0:
            ret = fItem._open * amount;
            break;
        case 1:
            ret = fItem._close * amount;
            break;
        case 2:
            ret = fItem._close_today * amount;
            break;
        default:
            ret = 0.0;
            break;
...
bool HisDataReplayer::cacheFinalBarsFromLoader(const std::string& key, const char* stdCode, WTSKlinePeriod period, bool bSubbed /* = true */)
{
    ...
    if (strlen(ruleTag) > 0) {
        ss << cInfo._exchg << "." << cInfo._product << "_" << ruleTag;
        if (cInfo.isExright())
...
bool HisDataReplayer::cacheIntegratedFutBarsFromBin(void* codeInfo, const std::string& key, const char* stdCode, WTSKlinePeriod period, bool bSubbed /* = true */)
{
    ...
    uint32_t endTDate = _bd_mgr.calcTradingDate(stdPID, curDate, curTime, false);
    ...
    if (cInfo->_exright == 1)
        ...
        barsList->_factor = secs.back()._factor;
