...
TraderFemas::TraderFemas()
    : m_pUserAPI(NULL), m_ayPosition(NULL), m_ayOrders(NULL), m_ayTrades(NULL), m_ayPosDetail(NULL), m_wrapperState(WS_NOTLOGIN), m_uLastQryTime(0), m_iRequestID(0), m_bQuickStart(false), m_bStopped(false), m_bQryOnline(false), m_uLocalOrdID(1), m_bInQuery(false), m_strandIO(NULL), m_lastQryTime(0)
{
}
    ...
    if (!IsErrorRspInfo(pRspInfo) && pRspInvestorPosition) {
        ...
        if (contract) {
            ...
            if (commInfo->getCategoty() != CC_Combination) {
                ...
                double availPre = pos->getNewPosition() + pos->getPrePosition() - pRspInvestorPosition->FrozenPosition - pos->getAvailNewPos();
...
WTSDirectionType TraderFemas::wrapDirectionType(TUstpFtdcDirectionType dirType, TUstpFtdcOffsetFlagType offsetType)
{
    if (USTP_FTDC_D_Buy == dirType)
        if (offsetType == USTP_FTDC_OF_Open)
            ...
            return WDT_SHORT;
    else if (offsetType == USTP_FTDC_OF_Open)
        return WDT_SHORT;
    else
        return WDT_LONG;
}
