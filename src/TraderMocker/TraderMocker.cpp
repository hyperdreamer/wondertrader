...
TraderMocker::TraderMocker()
    : _terminated(false), _listener(NULL), _ticks(NULL), _orders(NULL), _awaits(NULL), _trades(NULL), _b_socket(NULL), _max_tick_time(0), _last_match_time(0)
{
...
void TraderMocker::load_positions()
{
    ...
    if (root.HasMember("positions")) { // 读取仓位
