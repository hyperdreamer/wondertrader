...
bool cb_rtn_order(HANDLE_CONN hconn, HANDLE_SESSION hsess, int64_t subid, void* pData)
{
    ...
    */
    ...
    if (type == FIX_PUB_TYPE_COMM) { // 收到发布,一般用户在此应快速处理，如进队列，不应积压
        // printf("push order report begin ... \n");

        TraderDD* trader = (TraderDD*)pData;
        trader->OnRtnOrder(hconn, hsess);
    }
    else if (type == FIX_PUB_TYPE_RESUBS_BEFORE) { // 底层重订阅开始
    }
    else if (type == FIX_PUB_TYPE_RESUBS_AFTER_SUCC) { // 底层重订阅成功
    }
    else if (type == FIX_PUB_TYPE_RESUBS_AFTER_FAIL) { // 底层重订阅失败
    }
    else if (type == FIX_PUB_TYPE_DOWN) { // 网关中断或异常
    }
    else { // 未知
    }
    return true;
...
TraderDD::TraderDD()
    : m_hConn(NULL), m_wrapperState(WS_NOTLOGIN), m_uLastQryTime(0), m_bInQuery(false), m_strandIO(NULL), m_lastQryTime(0), m_orderRef(1), m_lDate(0), m_mapLives(NULL)
{
...
void TraderDD::reconnect()
{
    ...
    if (m_hConn == NULL) {
        ...
        StdThreadPtr thrd(new StdThread([this]() {
            ...
            if (m_traderSink)
                write_log(m_traderSink, LL_WARN, "[TraderDD]账号{}正在重连……", m_strUser.c_str());
            reconnect();
        }));
        return;
    }
...
int TraderDD::queryAccount()
{
    ...
    m_queQuery.push([this]() {
        ...
        if (code < 0) {
            ...
            if (m_traderSink)
                m_traderSink->onRspAccount(ayFunds);
            ayFunds->release();
        }
        Fix_ReleaseSession(sess);
        m_bInQuery = false;
...
int TraderDD::queryPositions()
{
    ...
    m_queQuery.push([this]() {
        ...
        if (!ret || code < 0) {
            ...
            if (m_traderSink)
                m_traderSink->onRspPosition(ayPositions);
            ayPositions->release();
        }
        Fix_ReleaseSession(sess);
        m_bInQuery = false;
...
int TraderDD::queryOrders()
{
    ...
    m_queQuery.push([this]() {
        ...
        if (code < 0) {
            ...
            if (m_traderSink)
                m_traderSink->onRspOrders(ayOrds);
            ayOrds->release();
        }
        Fix_ReleaseSession(sess);
        m_bInQuery = false;
...
int TraderDD::queryTrades()
{
    ...
    m_queQuery.push([this]() {
        ...
        if (code < 0) {
            ...
            if (m_traderSink)
                m_traderSink->onRspTrades(ayTrds);
            ayTrds->release();
        }
        Fix_ReleaseSession(sess);
        m_bInQuery = false;
