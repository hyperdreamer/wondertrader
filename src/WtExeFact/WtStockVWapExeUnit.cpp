...
WtStockVWapExeUnit::WtStockVWapExeUnit()
    : _last_tick(NULL), _comm_info(NULL) // 品种信息
      ,
      _ord_sticky(0), _cancel_cnt(0), _channel_ready(false), _last_fire_time(0), _fired_times(0), _total_times(0), _total_secs(0), _price_mode(0), _price_offset(0), _target_pos(0), _cancel_times(0), _begin_time(0), _end_time(0), _is_clear{ false }, _is_KC{ false }, isCanCancel{ true }
{
}
...
void WtStockVWapExeUnit::on_channel_ready()
{
    ...
    if (!decimal::eq(undone, 0) && !_orders_mon.has_order()) { // 未完成单不在监控中，撤单
        /*
         *	如果未完成单不为0，而OMS没有订单
...
void WtStockVWapExeUnit::on_tick(WTSTickData* newTick)
{
    ...
    if (isFirstTick) // 如果是第一笔tick,则检查目标仓位,不符合则下单
    {
        ...
        double realPos = _ctx->getPosition(stdCode);
        if (!decimal::eq(newVol, undone + realPos)) { // 如果是第一笔TICK，且目标量==未完成+仓位，退出
            do_calc();
        }
    }
    else {
