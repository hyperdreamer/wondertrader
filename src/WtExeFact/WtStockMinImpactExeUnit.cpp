...
WtStockMinImpactExeUnit::WtStockMinImpactExeUnit()
    : _last_tick(NULL), _comm_info(NULL), _price_mode(0), _price_offset(0), _expire_secs(0)
      //, _cancel_cnt(0)
      ,
      _target_pos(0), _cancel_times(0), _last_place_time(0), _last_tick_time(0), _is_clear{ false }, _min_order{ 0 }, _is_KC{ false }, _is_cancel_unmanaged_order{ true }, _is_finish{ true }, _is_first_tick{ true }, _is_ready{ false }, _is_total_money_ready{ false }
{
}
...
void WtStockMinImpactExeUnit::init(ExecuteContext* ctx, const char* stdCode, WTSVariant* cfg)
{
    ...
    ctx->writeLog(fmt::format("MiniImpactExecUnit {} inited, order price: {} ± {} ticks, order expired: {} secs, order timespan:{} millisec, order qty: {} @ {:.2f} min_order: {:.2f} is_cancel_unmanaged_order: {}",
                              stdCode, PriceModeNames[_price_mode + 1], _price_offset, _expire_secs, _entrust_span, _by_rate ? "byrate" : "byvol", _by_rate ? _qty_rate : _order_lots, _min_order, _is_cancel_unmanaged_order ? "true" : "false")
                      .c_str());
}
