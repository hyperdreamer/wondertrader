    ...
    if (bIsLast) {
        if (QRYFEES) {
            for (auto& v : _contracts) {
                ...
                if (!SET_FILTERS.empty() && SET_FILTERS.find(cInfo.m_strCode) == SET_FILTERS.end() && SET_FILTERS.find(cInfo.m_strProduct) == SET_FILTERS.end() && SET_FILTERS.find(cInfo.m_strAltCode) == SET_FILTERS.end())
                    continue;
