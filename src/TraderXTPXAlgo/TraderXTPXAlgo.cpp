...
inline WTSDirectionType wrapDirectionType(XTP_SIDE_TYPE side, XTP_POSITION_EFFECT_TYPE pe)
{
    if (XTP_SIDE_BUY == side)
        if (pe == XTP_POSITION_EFFECT_OPEN)
            ...
            return WDT_SHORT;
    else if (pe == XTP_POSITION_EFFECT_OPEN)
        return WDT_SHORT;
    else
        return WDT_LONG;
}
...
TraderXTPXAlgo::TraderXTPXAlgo()
    : _api(NULL), _sink(NULL), _ordref(makeRefID()), _reqid(1), _orders(NULL), _trades(NULL), _positions(NULL), _bd_mgr(NULL), _tradingday(0), _inited(false)
{
}
