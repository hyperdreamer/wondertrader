...
WtDataReader::WtDataReader()
    : _last_time(0), _base_data_mgr(NULL), _hot_mgr(NULL)
{
}
...
bool WtDataReader::loadStkAdjFactorsFromLoader()
{
    ...
    if (ret && _sink)
        pipe_reader_log(_sink, LL_INFO, "Adjusting factors of {} contracts loaded via extended loader", _adj_factors.size());
    return ret;
}
...
const WtDataReader::AdjFactorList& WtDataReader::getAdjFactors(const char* code, const char* exchg, const char* pid)
{
    ...
    if (it == _adj_factors.end()) {
        ...
        if (_loader) {
            if (_sink)
                pipe_reader_log(_sink, LL_INFO, "No adjusting factors of {} cached, searching via extented loader...", key);
