...
WtDataWriter::WtDataWriter()
    : _terminated(false), _save_tick_log(false), _log_group_size(1000), _disable_day(false), _disable_min1(false), _disable_min5(false), _disable_orddtl(false), _disable_ordque(false), _disable_trans(false), _disable_tick(false), _disable_his(false), _skip_notrade_tick(false), _skip_notrade_bar(false)
{
}
        ...
        _task_thrd.reset(new StdThread([this]() {
            while (!_terminated) {
                ...
                while (!tempQueue.empty()) {
                    ...
                    case 0:
                        procTick((WTSTickData*)curTask._obj, curTask._flag);
                        break;
                    case 1:
                        procQueue((WTSOrdQueData*)curTask._obj);
                        break;
                    case 2:
                        procOrder((WTSOrdDtlData*)curTask._obj);
                        break;
                    case 3:
                        procTrans((WTSTransData*)curTask._obj);
                        break;
                    default:
                        break;
                    }
...
void WtDataWriter::pipeToTicks(WTSContractInfo* ct, WTSTickData* curTick)
{
    ...
    if (blk && blk->_size >= blk->_capacity) {
        ...
        blk = pBlockPair->_block;
        if (blk)
            pipe_writer_log(_sink, LL_DEBUG, "RT tick block of {} resized to {}", ct->getFullCode(), blk->_capacity);
    }
...
void WtDataWriter::proc_loop()
{
    while (!_terminated) {
        ...
        if (fullcode.compare(CMD_CLEAR_CACHE) == 0) {
            ...
            ss_snapshot << "date,exchg,code,open,high,low,close,settle,volume,turnover,openinterest,upperlimit,lowerlimit,preclose,presettle,preinterest" << std::endl
                        << std::fixed;
