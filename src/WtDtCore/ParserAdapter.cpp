...
ParserAdapter::ParserAdapter(WTSBaseDataMgr* bgMgr, DataManager* dtMgr, IndexWorkerMgr* idxFactory)
    : _parser_api(NULL), _remover(NULL), _stopped(false), _bd_mgr(bgMgr), _dt_mgr(dtMgr), _idx_fact(idxFactory), _cfg(NULL)
{
}
        ...
        if (_parser_api->init(cfg)) {
            ...
            if (!_filters.empty()) {
                ...
                for (auto it = allContracts->begin(); it != allContracts->end(); it++) {
                    ...
                    if (it1 != _filters.end() || it2 != _filters.end() || it3 != _filters.end() || it4 != _filters.end() || it5 != _filters.end() || it6 != _filters.end())
