...
protected:
    ...
    inline int32_t translateIdx(int32_t idx) const
    {
        ...
        if (idx < 0) return max(0, totalCnt + idx);
        return idx;
    }
...
public:
    static WTSKlineSlice* create(const char* code, WTSKlinePeriod period, uint32_t times, 
                                 WTSBarStruct* bars = NULL, int32_t count = 0)
    {
        WTSKlineSlice *pRet = new WTSKlineSlice;
        wt_strcpy(pRet->_code, code);
        pRet->_period = period;
        pRet->_times = times;
        if (bars) pRet->_blocks.emplace_back(BarBlock(bars, count));
        pRet->_count = count;
    ...
    {
        if (bars == NULL || count == 0) return false;
    ...
    {
        if (blkIdx >= _blocks.size()) return NULL;
    ...
    {
        if (blkIdx >= _blocks.size()) return 0;
     ...
     *	并保存的一个数值数组中
     *	如果超出范围,则返回NULL
     *	@type 支持的类型有 KFT_OPEN、KFT_HIGH、KFT_LOW、KFT_CLOSE、KFT_VOLUME、KFT_DATE
     */
