...
public:
    ...
    constexpr inline double calcFee(double price, double qty, uint32_t offset) noexcept
    {
        ...
        if (m_nFeeAlg == 0) {
            ...
            switch (offset) {
            case 0:
                ret = m_dOpenFee * amount;
                break;
            case 1:
                ret = m_dCloseFee * amount;
                break;
            case 2:
                ret = m_dCloseTFee * amount;
                break;
            default:
                ret = 0.0;
                break;
...
protected:
    WTSContractInfo()
        : m_commInfo(NULL), m_openDate(19900101), m_expireDate(30991231), m_lMarginRatio(0), m_sMarginRatio(0), m_nFeeAlg(-1), m_uMarginFlag(0), m_uTotalIdx(UINT_MAX) {}
