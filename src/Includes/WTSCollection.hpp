...
public:
     ...
     *	grab接口读取数据以后,增加引用计数
     */
    inline WTSObject* at(uint32_t idx) noexcept
    {
    ...
    inline uint32_t idxOf(WTSObject* obj) noexcept
    {
        if (obj == NULL)
            return -1;
     ...
     *	用法同at函数
     */
    inline WTSObject* operator[](uint32_t idx) noexcept
    {
        if (idx < 0 || idx >= _vec.size())
            return NULL;
     ...
     *	增加引用计数
     */
    inline WTSObject* grab(uint32_t idx) noexcept
    {
        if (idx < 0 || idx >= _vec.size())
            return NULL;
     ...
     *	数据自动增加引用计数
     */
    inline void append(WTSObject* obj, bool bAutoRetain = true) noexcept
    {
        if (bAutoRetain && obj)
            obj->retain();
     ...
     *	新数据引用计数增加
     */
    inline void set(uint32_t idx, WTSObject* obj, bool bAutoRetain = true) noexcept
    {
        if (idx >= _vec.size())
            return;
    ...
    inline void append(WTSArray* ay) noexcept
    {
        if (ay == NULL)
            return;
...
public:
     ...
     *	用法同get函数
     */
    inline WTSObject* operator[](const T& _key) noexcept
    {
        Iterator it = _map.find(_key);
        if (it == _map.end())
            return NULL;
     ...
     *	如果key存在,则将原有数据释放
     */
    inline void add(T _key, WTSObject* obj, bool bAutoRetain = true) noexcept
    {
        if (bAutoRetain && obj)
            obj->retain();
     ...
     *	如果key存在,则对应数据引用计数-1
     */
    inline void remove(T _key) noexcept
    {
        Iterator it = _map.find(_key);
        if (it != _map.end()) {
    ...
    inline Iterator find(const T& key) noexcept
    {
    ...
    inline void erase(T key) noexcept
    {
        _map.erase(key);
    }
    ...
    inline WTSObject* last() noexcept
    {
        if (_map.empty())
            return NULL;
