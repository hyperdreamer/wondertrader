...
WtRtRunner::WtRtRunner()
    : _data_store(NULL), _cb_cta_init(NULL), _cb_cta_tick(NULL), _cb_cta_calc(NULL), _cb_cta_bar(NULL), _cb_cta_cond_trigger(NULL), _cb_cta_sessevt(NULL)

      ,
      _cb_sel_init(NULL), _cb_sel_tick(NULL), _cb_sel_calc(NULL), _cb_sel_bar(NULL), _cb_sel_sessevt(NULL)
      ...
      ,
      _cb_hft_orddtl(NULL), _cb_hft_ordque(NULL), _cb_hft_trans(NULL), _cb_hft_position(NULL), _cb_hft_sessevt(NULL)

      ,
      _cb_exec_cmd(NULL), _cb_exec_init(NULL)

      ,
      _cb_parser_evt(NULL), _cb_parser_sub(NULL)

      ,
      _cb_evt(NULL), _is_hft(false), _is_sel(false)

      ,
      _ext_fnl_bar_loader(NULL), _ext_raw_bar_loader(NULL), _ext_adj_fct_loader(NULL)
    ...
    try {
        ...
        if (!bAsync) {
            install_signal_hooks([this](const char* message) {
				if (!_to_exit)
					WTSLogger::error(message); }, [this](bool toExit) {
				if (_to_exit)
					return;
				_to_exit = toExit;
