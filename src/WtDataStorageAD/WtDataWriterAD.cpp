...
WtDataWriterAD::WtDataWriterAD()
    : _terminated(false), _log_group_size(1000), _disable_day(false), _disable_min1(false), _disable_min5(false), _disable_tick(false), _tick_cache_block(nullptr), _tick_mapsize(16 * 1024 * 1024), _kline_mapsize(8 * 1024 * 1024)
{
}
...
WtDataWriterAD::WtLMDBPtr WtDataWriterAD::get_k_db(const char* exchg, WTSKlinePeriod period)
{
    ...
    if (!dbPtr->open(path.c_str(), _kline_mapsize)) {
        if (_sink)
            pipe_writer_log(_sink, LL_ERROR, "Opening {} db at {} failed: {}", subdir, path, dbPtr->errmsg());
        return std::move(WtLMDBPtr());
    }
...
WtDataWriterAD::WtLMDBPtr WtDataWriterAD::get_t_db(const char* exchg, const char* code)
{
    ...
    if (!dbPtr->open(path.c_str(), _tick_mapsize)) {
        if (_sink)
            pipe_writer_log(_sink, LL_ERROR, "Opening tick db at {} failed: {}", path, dbPtr->errmsg());
        return std::move(WtLMDBPtr());
    }
