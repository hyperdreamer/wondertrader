...
TraderCTPOpt::TraderCTPOpt()
    : m_pUserAPI(NULL), m_mapPosition(NULL), m_ayOrders(NULL), m_ayTrades(NULL), m_ayPosDetail(NULL), m_wrapperState(WS_NOTLOGIN), m_uLastQryTime(0), m_iRequestID(0), m_bQuickStart(false), m_optSink(NULL), m_bscSink(NULL), m_bInQuery(false), m_bStopped(NULL), m_lastQryTime(0)
{
}
...
int TraderCTPOpt::orderInsertOpt(WTSEntrust* entrust)
{
    ...
    if (entrust->getBusinessType() != BT_EXECUTE) {
        if (m_bscSink)
            write_log(m_bscSink, LL_ERROR, "[TraderCTPOpt] Bz type not supported except option execution");
        return -1;
    }
        ...
        if (contract) {
            ...
            if (commInfo->getCategoty() != CC_Combination) {
                if (commInfo->getCoverMode() == CM_CoverToday) {
                    ...
                    double availPre = pos->getNewPosition() + pos->getPrePosition() - pInvestorPosition->LongFrozen - pInvestorPosition->ShortFrozen - pos->getAvailNewPos();
                    pos->setAvailPrePos(availPre);
                }
            }
            else {
