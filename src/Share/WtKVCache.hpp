...
private:
    bool resize(uint32_t newCap) noexcept
    {
        ...
        try {
            ...
            if (_logger)
                _logger("Got an exception while resizing cache file");
            return false;
        }
        ...
        try {
            if (!pNewMf->map(filename.c_str())) {
                delete pNewMf;
                if (_logger)
                    _logger("Mapping cache file failed");
                return false;
            }
        }
        catch (std::exception&) {
            if (_logger)
                _logger("Got an exception while mapping cache file");
            return false;
        }
...
public:
    bool init(const char* filename, uint32_t uDate, CacheLogger logger = nullptr) noexcept
    {
        ...
        if (!isNew && _cache._block->_date != uDate) {
            ...
            if (_logger)
                _logger("Cache file reset due to a different date");
        }
