...
public:
    ...
    void close()
    {
        if (_map_region) delete _map_region;
     
        if (_file_map) delete _file_map;
    ...
    void sync()
    {
        if (_map_region) _map_region->flush();
    }

    void* addr()
    {
        if (_map_region) return _map_region->get_address();
        return NULL;
    }

    size_t size()
    {
        if (_map_region) return _map_region->get_size();
        return 0;
    }

    bool map(const char *filename,
             int mode = boost::interprocess::read_write,
             int mapmode = boost::interprocess::read_write,
             bool zeroother = true)
    {
        ...
        if (_file_map == NULL) return false;
    ...
    const char* filename() { return _file_name.c_str(); }

    bool valid() const { return _file_map != NULL; }
